...
# init all clients
client_config = get_client_config(sandbox=False)
...
# 日志文件的路径
LOG_PATH = '/Users/tigeropen/sp500.log'
...
def recorder(func):
    ...
    def do_log(*args,**kwargs):
        try:
            ...
            log.warning(f"{func.__name__} is error,here are details:{traceback.format_exc()}")
    return do_log
...
def execute(target_weight):
    ...
    if len(current_holdings.keys()) != 0:
        ...
        latest_price = quote_client.get_stock_briefs(symbols=list(current_holdings.keys()))
        latest_price.set_index('symbol', inplace=True)
        ...
        for symbol in liquidation_list:
            try:
                ...
                order = limit_order(account=ACCOUNT,
                                    contract=contract,
                                    action='SELL',
                                    quantity=int(current_holdings[symbol]),
                                    limit_price=round(latest_price[symbol],2))
                trade_client.place_order(order)
                sell_orders.update({order.id:order})

            except Exception:
                log.error('Faild to get result', exc_info=True)
    ...
    latest_price = quote_client.get_stock_briefs(symbols=list(target_value.index))
    latest_price.set_index('symbol', inplace=True)
    latest_price = latest_price.latest_price
    ...
    for id in buy_orders.keys():
        ord = trade_client.get_order(account=ACCOUNT, id=id)
        buy_orders[id] = ord
